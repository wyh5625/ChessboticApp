...
void Game::movePiece(Position present, Position future, Chess::EnPassant* S_enPassant, Chess::Castling* S_castling, Chess::Promotion* S_promotion)
{
   ...
   // So, was a piece captured in this movePiece?
   if (0x20 != chCapturedPiece)
   {
      ...
      // Set Undo structure. If a piece was captured, then no "en passant" movePiece performed
   ...
   else if (true == S_enPassant->bApplied)
   {
      ...
      // Set Undo structure as piece was captured and "en passant" movePiece was performed
      m_undo.bCapturedLastMove = true;
   ...
   // Was it a castling movePiece?
   if ( S_castling->bApplied == true  )
   {
      // The king was already movePiece, but we still have to movePiece the rook to 'jump' the king
      ...
      // Save the 'CastlingAllowed' information in case the movePiece is undone
...
void Game::undoLastMove()
{
   ...
   // Since we want to undo a movePiece, we will be moving the piece from (iToRow, iToColumn) to (iFromRow, iFromColumn)
   ...
   // If a piece was captured, movePiece it back to the board
   if (m_undo.bCapturedLastMove)
   {
      ...
      // Move the captured piece back. Was this an "en passant" movePiece?
   ...
   // Finally, remove the last movePiece from the list
   deleteLastMove();
}
...
char Game::getPiece_considerMove(int iRow, int iColumn, IntendedMove* intended_move)
{
   ...
   // If there is no intended movePiece, just return the current position of the board
   ...
   {
      // In this case, we are trying to understand what WOULD happed if the movePiece was made,
      // so we consider a movePiece that has not been made yet
      if (intended_move->from.iRow == iRow && intended_move->from.iColumn == iColumn)
      {
         // The piece wants to movePiece from that square, so it would be empty
         chPiece = EMPTY_SQUARE;
      }
      else if (intended_move->to.iRow == iRow && intended_move->to.iColumn == iColumn)
      {
         // The piece wants to movePiece to that square, so return the piece
...
Chess::UnderAttack Game::isUnderAttack(int iRow, int iColumn, int iColor, IntendedMove* pintended_move)
{
   ...
   // a) Direction: HORIZONTAL
   {
      ...
      for (int i = iColumn + 1; i < 8; i++)
      {
         ...
         {
            // This square is empty, movePiece on
            continue;
         }
...
bool Game::isReachable( int iRow, int iColumn, int iColor )
{
   ...
   // a) Direction: HORIZONTAL
   {
      ...
      for (int i = iColumn + 1; i < 8; i++)
      {
         ...
         {
            // There is a piece that does not movePiece horizontally
            break;
         }
      }
   ...
   // b) Direction: VERTICAL
   {
      ...
      for (int i = iRow + 1; i < 8; i++)
      {
         ...
         {
            // There is a piece that does not movePiece vertically
            break;
         }
      }
   ...
   // c) Direction: DIAGONAL
   {
      ...
      for (int i = iRow + 1, j = iColumn + 1; i < 8 && j < 8; i++, j++)
      {
         ...
         {
            // There is a piece that does not movePiece diagonally
            break;
         }
      }
...
bool Game::isPathFree(Position startingPos, Position finishingPos, int iDirection)
{
   ...
   switch(iDirection)
   {
      ...
      {
         // If it is a horizontal movePiece, we can assume the startingPos.iRow == finishingPos.iRow
         // If the piece wants to movePiece from column 0 to column 7, we must check if columns 1-6 are free
      ...
      case Chess::VERTICAL:
      {
         // If it is a vertical movePiece, we can assume the startingPos.iColumn == finishingPos.iColumn
         // If the piece wants to movePiece from column 0 to column 7, we must check if columns 1-6 are free
      ...
      case Chess::DIAGONAL:
      {
         ...
         {
            throw("Error. Diagonal movePiece not allowed");
         }
      }
      break;
   }
...
bool Game::canBeBlocked(Position startingPos, Position finishingPos, int iDirection)
{
   ...
   switch(iDirection)
   {
      ...
      case Chess::DIAGONAL:
      {
         ...
         {
            cout << "Error. Diagonal movePiece not allowed\n";
            throw("Error. Diagonal movePiece not allowed");
         }
      }
      break;
...
bool Game::isCheckMate()
{
   ...
   // 2. Can the king movePiece the other square?
   ...
   for (int i = 0; i < 8; i++)
   {
      ...
      // Now, for every possible movePiece of the king, check if it would be in jeopardy
      // Since the movePiece has already been made, current_game->getCurrentTurn() now will return
...
bool Game::isKingInCheck(int iColor, IntendedMove* pintended_move)
{
   ...
   // Must check if the intended movePiece is to movePiece the king itself
...
void Game::logMove(std::string &to_record)
{
   ...
   {
      // If this was a white player movePiece, create a new round and leave the black_move empty
      Round round;
   ...
   else
   {
      ...
      // Remove the last round and put it back, now with the black movePiece
      rounds.pop_back();
      rounds.push_back(round);
   }
}
...
string Game::getLastMove(void)
{
   ...
   // Who did the last movePiece?
   if (BLACK_PLAYER == getCurrentTurn())
   {
      // If it's black's turn now, white had the last movePiece
      last_move = rounds[rounds.size() - 1].white_move;
   }
   else
   {
      // Last movePiece was black's
...
void Game::deleteLastMove( void )
{
   ...
   if (WHITE_PLAYER == getCurrentTurn())
   {
      // Last movePiece was white's turn, so simply pop from the back
      rounds.pop_back();
   }
   else
   {
      // Last movePiece was black's, so let's
      Round round = rounds[rounds.size() - 1];
      round.black_move = "";

      // Pop last round and put it back, now without the black movePiece
      rounds.pop_back();
      rounds.push_back(round);
   }
}
